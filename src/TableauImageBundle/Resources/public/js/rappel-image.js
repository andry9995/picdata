$(function() {
    ...
    var window_height = window.innerHeight,
        rappel_image_container = $('#rappel-image'),
        rappel_image_client = $('#client-rappel-image'),
        rappel_image_site = $('#site-rappel-image'),
        tableau_rappel_image = $('#js_rappel_image'),
        email_content = $('#email-content'),
        activer_envoi = $('#activer-envoi'),
        lastsel_rappel_image;
    var now = new Date();
    rappel_image_container.height(window_height - 200);
    var rappel_image_grid_width = rappel_image_container.width(),
        rappel_image_grid_height = rappel_image_container.height() - 115;
    ...
    /** SELECTION CLIENT */
    rappel_image_client.on('change', function(event) {
        var client_smtp = $('#client-smtp');
        event.preventDefault();
        getSites(rappel_image_client, rappel_image_site, null, null, function() {
            rappel_image_site.trigger('change');
        });
        var client = $(this).val();
        var url = Routing.generate('rappel_image_status_envoi', { client: client });
        fetch(url, {
            method: 'GET',
            credentials: 'include'
            ...
            if (data.sendNotificationImage === true) {
                if (activer_envoi.prop('checked') === false) {
                    activer_envoi.parent().find(".switchery").trigger("click");
                }
            } else {
                if (activer_envoi.prop('checked') === true) {
                    activer_envoi.parent().find(".switchery").trigger("click");
                }
            }
        ...
        var client_text = $(this).find('option:selected').text();
        var client_smtp_val = client_smtp.find('option').filter(function() {
            return $(this).html() === client_text;
        }).val();
        if (changeCount > 0) {
            client_smtp.val(client_smtp_val).trigger('change');
        }
        changeCount++;
    });

    rappel_image_client.trigger('change');

    /** SELECTION SITE */
    rappel_image_site.on('change', function(event) {
       event.preventDefault();
            ...
            preConfirm: function() {
                return new Promise(function(resolve) {
                    var client = rappel_image_client.val(),
                        ...
                        url = Routing.generate('rappel_image_param_edit_all'),
                        formData = new FormData();
                    formData.append('client', client);
                    formData.append('site', site);
    ...
    function updateGridSizeRappelImage() {
        setTimeout(function() {
            ...
            rappel_image_container.height(window_height - 200);
            rappel_image_grid_width = rappel_image_container.width();
            rappel_image_grid_height = rappel_image_container.height() - 115;
            tableau_rappel_image.jqGrid("setGridWidth", rappel_image_grid_width);
