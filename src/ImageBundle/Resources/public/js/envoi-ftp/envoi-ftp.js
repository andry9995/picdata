/**
 * Created by TEFY on 04/04/2017.
 */
$(function() {
    var grid_envoi_ftp = $('#js_envoi_ftp'),
    ...
    var text_fini = '<i class="fa fa-check"></i> Numérotation finie';
    var text_original = '<i class="fa fa-check"></i> Numéroter';
    var text_busy = '<i class="fa fa-cog fa-spin fa-fw"></i> Numérotation...';

    //jqGrid Liste images sur FTP
    ...
    $(document).on('click', '#btn-init', function(event) {
        ...
        if (!numerotation_en_cours) {
            swal({
                ...
                clearGrid(grid_envoi_ftp);
                grid_envoi_ftp.setGridParam({
                    url: Routing.generate('img_envoi_ftp_liste', {init: 1}),
                    datatype: 'json',
                    loadonce: true,
                    page: 1
                }).trigger('reloadGrid');
    ...
    $(document).on('click', '#btn-numeroter', function(event) {
        ...
        if (!numerotation_en_cours) {
            ...
            var url = Routing.generate('img_envoi_ftp_numeroter');
            fetch(url, {
                method: 'POST',
                credentials: 'include'
